...
# Parallel GC Throughput Test Runner
# This script runs the GC test server with Parallel Garbage Collector
...
PORT="${PORT:-8081}"
GC_LOG_FILE="$LOG_DIR/parallel-gc-$(date +%Y%m%d-%H%M%S).log"

# Create directories
mkdir -p "$LOG_DIR"

echo "=============================================="
echo "Starting GC Throughput Test with Parallel GC"
echo "=============================================="
...
# Parallel GC specific JVM options
JVM_OPTS=(
    # Memory settings
    "-Xms${HEAP_SIZE}"
    "-Xmx${HEAP_SIZE}"

    # Parallel GC selection
    "-XX:+UseParallelGC"

    # Parallel GC tuning options
